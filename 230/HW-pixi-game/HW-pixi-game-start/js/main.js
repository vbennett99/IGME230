...
function gameLoop(){
    ...
    let amt = 6 * dt; // at 60 fps would move about 10% of the distance per update
    
    // linear interpolate the x and y values with lerp()
...
function end() {
    paused = true;
    // clear out level
    circles.forEach(c=>gameScene.removeChild(c)); // concise arrow function with no brackets or return
    circles = [];
...
function fireBullet(e){
    ...
    if(score > 5) {
        // another bullet on the right
        let b2 = new Bullet(0xFFFFFF, ship.x + 10, ship.y);
        bullets.push(b2);
        // another bullet on the left
    ...
    // and this will run reguardless of score
